...
void AEasingTest::ScaleLerp()
{
	...
	/*
	currScale 
	= FMath::Lerp(1.0f,2.0f, ratio);
	FVector scale = FVector::OneVector * currScale;
	SetActorScale3D(scale);
	*/

	FVector scale =
	FMath::Lerp(FVector::OneVector, FVector::OneVector * 2, ratio);
	SetActorScale3D(scale);
}	

void AEasingTest::ScaleEase()
{
	ratio += GetWorld()->GetDeltaSeconds() *3; //하면 속력 조절되 됨
	if(ratio > 1) ratio = 1;

	float s = UEasingLibrary::EaseOutBounce(ratio);
